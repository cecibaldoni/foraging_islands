...
foraging_master <- foraging_master %>%
  ...
  mutate(first_AD_time  = first_AD_frame * (1/24))
...
foraging_master <- foraging_master %>%
  mutate(travelling_time = travelling_frames * (1/24))
...
foraging_master <- foraging_master %>%
  mutate(islands_time = island_frame * (1/24))


#Ordering the columns
foraging_master <- foraging_master %>%
  select(unique_trial_ID, season, ID, trial, first_AD_frame, first_AD_time, A_0, A_1, B_0, C_0, D_0, D_1)
